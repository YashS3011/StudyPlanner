...
class SubjectsProvider with ChangeNotifier {
  ...
  Future<void> updateTopicProgress(String id, int progress) async {
    try {
      // Auto-update status based on progress
      TopicStatus status = TopicStatus.inProgress;
      if (progress == 0) status = TopicStatus.notStarted;
      if (progress == 100) status = TopicStatus.completed;

      await SupabaseConfig.client.from('topics').update({
        'progress': progress,
        'status': status.value,
      }).eq('id', id);
      ...
      final index = _topics.indexWhere((t) => t.id == id);
      if (index != -1) {
        _topics[index] = _topics[index].copyWith(progress: progress, status: status);
        notifyListeners();
      }
    } catch (e) {
      rethrow;
  ...
  double getSubjectCompletion(String subjectId) {
    ...
    final totalProgress = subjectTopics.fold<int>(0, (sum, t) => sum + t.progress);
    return totalProgress / (subjectTopics.length * 100);
  }
}
